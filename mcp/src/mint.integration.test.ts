...
describe('POST /admin/keys', () => {
  it('mints a key for the JWT user and stores only its hash', async () => {
    ...
    const row = await admin
      .from('api_keys')
      .select('key_hash, name, user_id, revoked')
      .eq('key_hash', hashApiKey(key, config.apiKeyPepper))
      .single<{ key_hash: string; name: string; user_id: string; revoked: boolean }>();
  ...
  it('never stores the plaintext key in any api_keys row', async () => {
    ...
    const all = await admin.from('api_keys').select('key_hash, name').eq('user_id', userA.userId);
  ...
  it('rejects a request with an invalid JWT', async () => {
    expect((await mint({ Authorization: 'Bearer not-a-jwt' })).status).toBe(401);
  });

  it('rejects a body carrying a user_id selector', async () => {
    const res = await mint({ Authorization: `Bearer ${userA.accessToken}` }, { user_id: 'other' });
