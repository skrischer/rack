...
| Admin key-mint endpoint authenticated by the Supabase user JWT (verified against `SUPABASE_JWT_SECRET`), not by an API key; locally the JWT comes from signing up against the local Auth | The app holds a user JWT, not an API key, when a user creates their first key (bootstrapping); the JWT's `sub` is the only identity the mint trusts, so the endpoint also takes no `user_id` from the caller. | 2026-06-17 |
