...
- [ ] wger CC-BY-SA images are stored in a dedicated public-read Supabase Storage bucket; the catalog's `image_path` resolves to a public URL the Android client loads with the anon key only.
...
- A `/supabase/migrations` migration that enriches the global `exercises` catalog with `instructions`, `primary_muscles`, `secondary_muscles`, and `image_path`, **reusing** the existing `muscles`/`equipment`/`category`/`license`/`license_author` columns from Phase 1 (no duplicate `muscles` or `equipment` column is added), keeping the catalog public-read (no owner RLS — it is global seed data, not user-owned).
- A Supabase Storage public bucket for wger CC-BY-SA exercise images, with a read policy the anon role can use.
- A repeatable seed/enrichment routine (under `/supabase`) that: imports wger instructions + images with attribution, complements instruction text from the free-exercise-db public-domain JSON only where wger is missing, and explicitly excludes free-exercise-db images.
- An Android exercise detail feature (screen + ViewModel + repository method) reachable from the existing plan/day/exercise view, in the Recomp design language.
...
- [ ] Confirm the managed Supabase project is provisioned and reachable (URL + anon + service-role keys in `.env`) so the catalog-enrichment migration and Storage bucket can be created (carried from Phase 1).
- [ ] Confirm the Phase 1 wger import already populated `exercises` with `name`, `muscles`, `equipment`, `category`, `license`, and `license_author` (per `../architecture.md` data model) so this phase enriches existing rows rather than seeding from scratch; provide the wger export (CC-BY-SA images + instruction texts) reachable by the seed-enrichment script.
- [ ] Provide the free-exercise-db `exercises.json` (Unlicense / public-domain) snapshot for text-only gap-filling; confirm NO image files from free-exercise-db are included in the import (image license unresolved).
...
| wger images go into a dedicated public-read Supabase Storage bucket; catalog stores a relative `image_path`, the app resolves the public URL | Storage is the architecture's chosen media store; a public bucket lets the anon client load images without a service-role key or signed URLs. The column name is `image_path` everywhere. | 2026-06-17 |
...
Per the workflow contract, `Test` is `none yet`, so machine gates plus enumerated human-QA checks apply.
...
- Human QA: open the app, navigate plan → day → tap an exercise; the detail screen opens showing name, category, primary/secondary muscles, equipment, ordered instruction steps, and an image or placeholder.
- Human QA: pick an exercise known to come from wger — its detail screen shows a visible CC-BY-SA attribution line with the author.
- Human QA: pick an exercise where wger text was missing — its instructions are present (from free-exercise-db) and the screen renders without a blank panel.
- Human QA: pick an exercise with no licensed image — it shows the deterministic placeholder, and the same exercise shows the same placeholder on re-entry.
- Human QA: the detail screen matches the Recomp dark theme, volt accent, fonts, and tag coloring of the plan/day view; images load over the anon key with no auth error.
...
| Enrichment re-run duplicates rows or clobbers Phase 1 seed | Routine is idempotent/additive keyed on the stable wger id (name fallback); run-twice verification step; no base re-import. |
...
| Public Storage bucket exposes more than intended | Bucket holds only attributed CC-BY-SA exercise images; read-only public policy, no user data in the bucket. |
| Catalog table mistakenly given an owner RLS policy and breaking public reads | Migration documents and ships a public SELECT policy; QA confirms the anon client reads detail data without a JWT. |
...
- Use a dedicated public-read Supabase Storage bucket for wger CC-BY-SA images with a relative `image_path` resolved to a public URL client-side — `image_path` is the column name everywhere (auto-resolved under autonomous planning mandate, 2026-06-17).
