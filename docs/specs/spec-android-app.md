...
Build the native Android app (Kotlin + Jetpack Compose, MVVM, Coroutines/Flow) that logs the user into Supabase Auth, reads their plans/days/exercises under RLS through a `supabase-kt` repository layer, and lets them log sets (kg + per-set reps + RIR) with dated history in the Recomp design language — with a light local cache for unsynced logs and `assembleDebug` building green. Built and verified now against the **local Supabase stack** (`supabase start` in Docker: API at `http://localhost:55321`, generated anon/service-role keys, local Postgres, local Realtime + Storage); the emulator reaches it at `http://10.0.2.2:55321`. The managed Supabase project remains the eventual production target (deferred — hosted deploy).
...
- [ ] The app reads `supabase.url` and `supabase.anonKey` from `android/local.properties` (gitignored) into `BuildConfig` at build time; for local dev these are `http://10.0.2.2:55321` (emulator) and the **local** anon key printed by `supabase start`; no Supabase URL/key and no service-role key or other secret is committed anywhere under `/android`.
...
- [ ] Launching the app while signed out shows a login screen; entering valid email/password signs in via Supabase Auth, persists the session across app restarts, and routes to the plan view; invalid credentials show an inline error and no crash. The test user is created by signing up against the **local** Auth (no dashboard needed).
...
- `supabase-kt` integration: a single shared `SupabaseClient` (Auth, Postgrest, Realtime, Storage installed) configured from `BuildConfig` values sourced from `android/local.properties`, pointed at the local stack (`http://10.0.2.2:55321` + local anon key) for dev.
- Email/password authentication against Supabase Auth: login screen, session persistence across restarts, sign-out, signed-in vs signed-out routing. The QA test user is created by signing up against the local Auth.
...
- Realtime subscription and `source`-based agent-edit highlighting — that is the wow loop of [Phase 4 (live-sync)](docs/specs/spec-live-sync.md); this phase only writes `source = 'app'` and reads, it does not subscribe or highlight. (Local Realtime is available in the local stack and is consumed in Phase 4.)
...
- Sign-up / password-reset / email-confirmation flows beyond what a test user needs — friends-and-family accounts are provisioned out of band (vision: no OAuth/compliance build-out); for local dev the test user self-signs-up against local Auth.
- The MCP server, hosting, and the database schema/RLS themselves — owned by Phases 1 and 2; this phase consumes the schema (applied via migrations against the local Docker DB) from [Phase 1 (foundation)](docs/specs/spec-foundation.md).
- Production hosting — the managed Supabase project, the VPS + Caddy + domain/TLS for the MCP, and any FCM/push delivery — is **deferred (hosted deploy)**; the constitution's hosting choices remain the eventual target but no hosted infra is required to build or verify this phase.
...
Local-dev constraint: the app is wired to the local stack (`http://10.0.2.2:55321` from the emulator, or the host LAN IP from a physical device) with the **local** anon key only; the URL/key are swapped to the managed project for production with no code change (deferred — hosted deploy). Known local caveat: some MCP clients want HTTPS even for localhost — this affects only the (out-of-phase) MCP client, not the app, and is solvable with a local `cloudflared` quick-tunnel or a local Caddy localhost cert; it is not a blocker for this phase.
...
### Local dev (what this phase actually needs now)

- [ ] Dev tooling on the build machine: JDK 17 and the Android SDK (or Android Studio) so `./gradlew assembleDebug` runs, plus a device or emulator (API 26+) to install and smoke-test the app. Environmental, not hosted infra.
- [ ] Docker available so `supabase start` can bring up the local stack (API at `http://localhost:55321`, generated anon/service-role keys, local Postgres, local Realtime + Storage). The **local** anon key it prints goes into `android/local.properties` as `supabase.anonKey`; the URL is `http://10.0.2.2:55321` for the emulator.

No human-delivered secrets are required for local development: the local stack generates its own keys, the QA test user is created by signing up against local Auth, and seed plan/day/`plan_exercises`/`set_logs` data is inserted via the local rack-MCP or a direct insert against the local DB. (Schema is applied by Phase 1's migrations against the local Docker DB.)

### Deferred — hosted deploy (recorded, not blocking local work)

- [ ] Managed Supabase project URL + anon key for production `android/local.properties` (the only credentials the production app needs; no service-role key in the client).
- [ ] A managed-project test user (or confirmed sign-up) and seeded plan data under RLS in the managed project, for production QA.
- [ ] VPS + domain + DNS + TLS/Caddy fronting the hosted rack-MCP (production MCP only; the app never holds its secrets).
- [ ] Firebase project + FCM service account + a Play-services device for real push delivery (only relevant to later push-notification phases, not this one).

## Prior decisions

| Decision | Rationale | Date |
| --- | --- | --- |
| Local-first dev path: build and verify against `supabase start` (local Docker stack), emulator at `http://10.0.2.2:55321` + local anon key; hosted/managed deployment deferred | No managed project, VPS, or human-delivered secret is needed to develop or QA this phase; the local stack provides Auth, Postgrest, Realtime, Storage, and a generated key set. The managed project is a credential swap later. | 2026-06-17 |
...
| Supabase credentials read from `android/local.properties` → `BuildConfig`, never committed | The Phase-1 `.env.example` already prescribes `supabase.url` / `supabase.anonKey` in `local.properties`; constitution forbids secrets in the client and only the anon key is client-safe. The same mechanism carries the local key now and the managed key in production. | 2026-06-17 |
...
| Login UI is email/password only; no sign-up/reset in-app this phase | Vision: friends-and-family, no OAuth/compliance build-out; accounts are provisioned out of band (locally, self-signed-up against local Auth), so the app only needs sign-in + sign-out. | 2026-06-17 |
...
Machine gates (from `docs/workflow.md`): `make verify` (Android ktlint/detekt static checks) green, and `make build` (`cd android && ./gradlew assembleDebug`) produces a debug APK. `Test` is `none yet`, so the following behavioral checks are run by a human at the milestone-QA gate on a device/emulator (API 26+) signed in to a test user against the **local** stack (`supabase start` running; emulator reaching `http://10.0.2.2:55321`):

- Clean-checkout build: with valid `android/local.properties` (local URL + local anon key), `make build` produces an installable debug APK; `git status` shows no secret files would be committed (`local.properties` is gitignored).
- Login: signed-out launch shows the login screen; valid credentials (the locally self-signed-up test user) route to the plan view; wrong password shows an inline error and no crash; killing and relaunching the app stays signed in; sign-out returns to login.
...
- Logging: entering kg + per-set reps and tapping log writes a row, the entry appears immediately in that exercise's history, and a separate MCP read (or a query against the local Supabase DB) confirms the row with `source = 'app'` and the correct `weight`/`reps[]`/`rir`/date.
...
- (deferred — hosted deploy) Re-run the build + login + read-back checks against the managed Supabase project by swapping `local.properties` to the production URL + anon key, confirming no code change is needed.
...
| Phase 1's schema (`set_logs` columns, `reps[]` type, `source` default, RLS) differs from this spec's assumptions. | Read the merged Phase 1 migration in `/supabase/migrations` (applied against the local Docker DB) before implementing the repository; map Kotlin models to the actual `snake_case` columns; treat any mismatch as a `needs:planning` escalation rather than a workaround. |
| Postgres array (`reps[]`) (de)serialization through Postgrest/`supabase-kt` is fiddly. | Model `reps` as `List<Int>` with an explicit serializer matching the column type; cover it in the manual read-back QA check (log then read via MCP/local DB query). |
...
| Emulator cannot reach the local stack (wrong host). | Use `http://10.0.2.2:55321` from the emulator (not `localhost`), or the host LAN IP from a physical device; documented in `local.properties` guidance and checked in the login QA step. |
| Secret leakage into the repo (URL/anon key hard-coded). | `local.properties` is gitignored and the only source of the values via `BuildConfig`; a QA step asserts no Supabase values are tracked by git. Only the local anon key is used in dev. |
| Scope creep into Realtime highlighting (Phase 4). | Explicitly out of scope; the repository exposes plain reads/writes with `source='app'`, leaving the subscription seam for Phase 4 (local Realtime is available when that phase consumes it). |
...
- (local-first dev path adopted 2026-06-17; hosted deploy deferred) Build and verify against the local Supabase stack (`supabase start`, emulator at `http://10.0.2.2:55321`, local anon key, local Realtime/Storage); managed Supabase project, VPS + Caddy + domain/TLS, and FCM remain the eventual production target but are not required for this phase.
- Single Activity + Compose Navigation; no fragments. (auto-resolved under autonomous planning mandate, 2026-06-17)
- Supabase URL/anon key flow `android/local.properties` → `BuildConfig`; nothing secret committed; local key now, managed key in production. (auto-resolved under autonomous planning mandate, 2026-06-17)
...
- Email/password sign-in + sign-out only; no in-app sign-up/reset/confirmation this phase (local test user self-signs-up against local Auth). (auto-resolved under autonomous planning mandate, 2026-06-17)
